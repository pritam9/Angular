...
public class UpdateProfileServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String credicard = request.getParameter("creditCard");
        MyResponse myResponse=new MyResponse();
        response.setContentType("application/json");
        PrintWriter out = response.getWriter();
        Customer customer = new Customer(email, firstName, lastName, phone, credicard);
        if(new CustomerDaoImpl().updateCustomer(customer)){
            myResponse.setData(customer);
            myResponse.setMessage("Profile updated successful.");
            myResponse.setStatus(MyResponse.SUCCESS);
        }else{
            myResponse.setData(null);
            myResponse.setMessage("Profile update failed!!");
